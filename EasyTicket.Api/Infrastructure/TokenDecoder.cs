...
";
        ...
        private const string TokenPartPattern = @"(\$\$_=~.*;)";
        private const string TokenPattern = @"""([^\s]*)""\);";

        public string Decode(string pageHtml) {
            string encodedTokenPart = FindEncodedTokenPart(pageHtml);
            using (var engine = new ScriptEngine("jscript")) {
                ParsedScript parsed = engine.Parse(JsCode);
                object tokenPart = parsed.CallMethod("jjdecode", encodedTokenPart);
                return FindToken(tokenPart.ToString());
            }
        }

        private string FindEncodedTokenPart(string pageHtml) {
            var regex = new Regex(TokenPartPattern, RegexOptions.Multiline);
            Match match = regex.Matches(pageHtml).OfType<Match>().FirstOrDefault();
            return match?.Value ?? "";
        }
